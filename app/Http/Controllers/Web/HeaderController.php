...
class HeaderController extends Controller
{
    public function show(Request $request,$slug)
    {
     ...
     if($request->ajax())
     {
        ...
        $slugs = Str::slug($slug, '-');
        if($slugs == 'conservatory-blinds')
        {
          $data['header'] =   DB::select(DB::raw("SELECT a.`id`,a.`slug`, a.`title`,b.`image_path` as b_image ,b.`slug` as slug_title, b.id as sub_id, b.title as sub_title FROM `headers` a LEFT JOIN `headers` b on a.id = b.parent_header_id  WHERE  a.`status` = 1 and a.`slug`='$slugs'"));
          return view('web.menus', $data);
        }
        else{
          $data['header'] =  DB::select(DB::raw("SELECT a.`id`, a.`title` ,b.`slug` as slug_title, b.id as sub_id, b.title as sub_title ,c.`slug` as slug_c_title,c.id as sub_c_id,c.`image_path` as c_image , c.title as sub_c_title FROM `headers` a LEFT JOIN `headers` b on a.id = b.parent_header_id LEFT JOIN `headers` c on b.id = c.parent_header_id WHERE  a.`status` = 1 and b.`slug`='$slugs  '
            "));
          return view('web.blinds.blind-submenu-view',$data);
        }
    }
...
public function showServices($slug)
{
    ...
    if($slug == 'privacy-policy' || $slug == 'terms-conditions' || $slug == 'connect-via-zoom')
    {
    ...
    ->where('pages.status', '1')
    ->firstOrFail();
    $data['headers'] =  DB::select(DB::raw("SELECT a.`id`, a.`title` ,b.`slug` as slug_title, b.id as sub_id, b.title as sub_title ,c.`slug` as slug_c_title,c.id as sub_c_id,c.`image_path` as c_image , c.title as sub_c_title FROM `headers` a LEFT JOIN `headers` b on a.id = b.parent_header_id LEFT JOIN `headers` c on b.id = c.parent_header_id WHERE  a.`status` = 1 and b.`slug`='$slug'
        "));
